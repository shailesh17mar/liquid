...
function createView(
  ...
  view.webContents.on("dom-ready", () => {
    view.webContents.executeJavaScript(
      "localStorage.webviewId = " + id.toString() + ";"
    );
  });
...
function addView(id) {
  mainWindow.addBrowserView(viewMap[id]);
  console.log("add view called ", id);
}
...
function changePositions(movement) {
  for (const id in viewMap) {
    const view = viewMap[id];
    const bounds = view.getBounds();
    const newBounds = { ...bounds, x: bounds.x - movement };
    viewMap[id].setBounds(newBounds);
  }
}
...
ipc.on("changePositions", function (e, args) {
  changePositions(args.movement);
});
...
ipc.on("setBounds", function (e, args) {
  console.log("set bounds ", args.bounds);
  setBounds(args.id, args.bounds);
});
