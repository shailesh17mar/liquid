...
class Liquid {
  ...
  // private readonly host: string = 'wss://fluidguild.ml/';
  private readonly host: string = 'ws://localhost:8080/';
  ...
  private constructor(metadata: IMetadata) {
    // this.host = "ws://liquid-guild.eastus.cloudapp.azure.com/";
    // this.host = 'wss://fluidguild.ml/';
...
<path d="M15.4098 6.50977L8.58984 10.4898" stroke="#828282" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
  ...
  private syncScreens() {
    this.ws.send(
      JSON.stringify({
        type: `${this.MESSAGE_ENUM.SYNC_SCREENS}:${this.connectionHash}`,
        body: {
          w: window.innerWidth,
  ...
  private addUser(id: string, name: string, color: string) {
  ...
  private initConnection = () => {
    //send a message to background thread
    // return new WebSocket(
    //   `${this.host}${
    //     this.metadata
    //       ? `?token=${this.metadata.token}&url=${window.location.href}`
    //       : ''
    //   }`
    // );
    ...
    if (this.isPing(data)) {
      ...
      switch (type) {
        case this.MESSAGE_ENUM.SELF_CONNECTED:
          ...
          this.setUser(JSON.stringify(body));
          if (this.isRealtimeCollabEnabled) this.startTracking();
          this.addUser(id, body.name, body.color);
          break;
